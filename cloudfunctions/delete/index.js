...
//以下全部data内容
  ...
  var type111=event._data.type111
...
//以上全部data内容
  ...
  if(type111=="tianmeizhoubian"){
  if(id1==""){
    //这是删除lv0评论
    cloud.database().collection('tianmeizhoubian').doc(id).get().then((res)=>{
    ...
    cloud.database().collection('tianmeizhoubian').where({
      _id:id,
